...
while in uncontrolled mode it has `defaultValue` and `key` props.

Uncontrolled mode relies on `key` returned from `form.getInputProps` to update
components when `form.setFieldValue` or `form.setValues` are called. You should
not override `key` supplied by `form.getInputProps`:
...
function Demo() {
  ...
  // ❌ Do not override key prop
  return <input {...form.getInputProps('text')} key="my-key" />;
}
```
...
function Demo() {
  ...
  const fields = form.getValues().jobs.map((item, index) => (
      <div key={item.key}>
        <input {...form.getInputProps(`jobs.${index}.company`)} />
      </div>
    ));
...
function Demo() {
  ...
  return <CustomInput {...form.getInputProps('text')} />;
}
```
