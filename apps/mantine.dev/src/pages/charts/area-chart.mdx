...
Set `type="stream"` to render a streamgraph (also known as ThemeRiver). A streamgraph is a
stacked area chart whose baseline flows around a central axis instead of being fixed to zero,
producing the characteristic organic "river" shape. It is useful for showing how the composition
of a total shifts over time when the overall flow and proportions matter more than exact values.

Since the baseline floats, the y-axis values are not meaningful to read off – the y-axis is
hidden by default for `type="stream"`. You can set `withYAxis` back to `true` if you need it.
...
the full opposite axis, for example provide only `x1`/`x2` to highlight a time range across the
entire height of the chart. Set `color` to any [theme color](/theming/colors/) or CSS color and
`label` to display text inside the area. Reference areas are rendered behind the chart series.
...
<Demo data={AreaChartDemos.brushProps} />
