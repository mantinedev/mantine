...
the full opposite axis, for example provide only `x1`/`x2` to highlight a range of categories
across the entire height of the chart. Set `color` to any [theme color](/theming/colors/) or CSS
color and `label` to display text inside the area. Reference areas are rendered behind the bars.

<Demo data={BarChartDemos.referenceAreas} />
...
<Demo data={BarChartDemos.referenceDots} />
