...
`use-floating-window` makes given element draggable:
...
`UseFloatingWindowOptions` and `UseFloatingWindowReturnValue` types are exported from `@mantine/hooks` package,
