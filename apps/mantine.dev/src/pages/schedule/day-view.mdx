...
`eventOverlapMode` prop controls how events that overlap in time share the horizontal space:

- `columns` (default) – the available width is split evenly between the overlapping events. Every event
  is fully visible, but each one gets narrower as more events overlap.
- `cascade` – each event is indented from the previous one and drawn on top of it, extending to the
  right edge of the group. Events keep a readable width no matter how many of them overlap, at the cost
  of covering the events underneath.

Prefer `cascade` when your data regularly produces large groups of simultaneous events, where `columns`
would shrink each event to an unreadable sliver.
...
Covered events can still be read and interacted with: rest the pointer on one and it is raised above
the events covering it, which also makes its resize handles reachable. Keyboard users get the same
result by focusing the event with `Tab`, without any delay.

`eventOverlapRaiseDelay` prop controls how long the pointer must rest on an event before it is raised,
in milliseconds. The default of `600` is long enough that moving the pointer across a group to reach a
specific event does not raise the events it passes over. Set a smaller value to make covered events
reveal themselves sooner:

```tsx
import { DayView } from '@mantine/schedule';

function Demo() {
  return <DayView date={new Date()} eventOverlapMode="cascade" eventOverlapRaiseDelay={250} />;
}
```
...
unavailability, lunch breaks, focus time, or other blocked periods. By default they
cannot be clicked, dragged, or resized.
...
Note that an interactive background event covers the time slots underneath it: clicks and
drag-to-select in its range go to the event instead of the grid. Interactive background
events still cannot be dragged.

The prop is supported by `DayView`, `WeekView`, `MonthView`, `ResourcesDayView` and
`ResourcesWeekView`. `YearView` and the `MobileMonthView` that `Schedule` renders with
`layout="responsive"` on small screens do not display background events at all, so the prop
has no effect there.
...
events render resize handles on their top and bottom edges. Dragging a handle calls
...
<Demo data={DayViewDemos.resizableBackgroundEvents} />
