...
interface ScheduleEventBase<Payload extends EventPayload = EventPayload> {
  ...
  /** Event display mode. Background events render as full-width blocks behind regular
   *  events, non-interactive unless `withInteractiveBackgroundEvents` is set on the view.
   *  @default 'default' */
