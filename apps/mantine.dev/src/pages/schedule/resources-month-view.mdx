...
Resize handles are rendered only on the edges where the event actually starts and ends. An event
that continues outside the displayed month, or that is split across rows, does not render a handle
on those edges. Events hidden behind the "+N more" indicator cannot be resized.
