...
Pass a [theme object](/theming/theme-object) override to the `theme` prop. It will be merged with the default
...
By default, it is `:root`. `MantineProvider` generates CSS variables based on the given [theme override](/theming/theme-object/)
and `cssVariablesResolver`, then these variables are rendered into a `<style />` tag next to your application.
...
In this case (default `classNamesPrefix`), the [Text](/core/text) component will have the following classes:
...
- `mantine-Text-root` – component static class, part of the [Styles API](/styles/styles-api)
...
Now the [Text](/core/text) component will have the following classes:
...
Note that `env="test"` is intended to be used in test environments only with [Jest](/guides/jest) or [Vitest](/guides/vitest). Do not use it in
