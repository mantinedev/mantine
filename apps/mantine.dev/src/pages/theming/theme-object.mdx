...
To customize the theme, pass a theme override object to [MantineProvider](/theming/mantine-provider/) `theme` prop.
...
except for [Spotlight](/x/spotlight) and [@mantine/dates](/dates/getting-started) components which only support the global theme setting.
...
`theme.components` allows overriding of components' [default props](/theming/default-props) and styles with `classNames` and `styles` properties.
You can learn more about these features in the [default props](/theming/default-props) and [Styles API](/styles/styles-api) guides.
...
The `useMantineTheme` hook returns the theme object from [MantineProvider](/theming/mantine-provider) context:
