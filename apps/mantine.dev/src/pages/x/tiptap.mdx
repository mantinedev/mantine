...
To add tables support, install the [Tiptap table extension](https://tiptap.dev/docs/editor/extensions/nodes/table)
...
`RichTextEditor` provides the following table controls. They are automatically disabled when the
cursor is not inside a table (or when the table extension is not installed):

- `RichTextEditor.TableInsert` – inserts a table, opens a grid to pick its size. Supports `maxColumns`,
  `maxRows`, `withHeaderRow` and `popoverProps` props.
- `RichTextEditor.TableDelete` – deletes the entire table
- `RichTextEditor.TableColumnBefore` / `RichTextEditor.TableColumnAfter` – add a column before/after
- `RichTextEditor.TableColumnDelete` – deletes the current column
- `RichTextEditor.TableRowBefore` / `RichTextEditor.TableRowAfter` – add a row before/after
- `RichTextEditor.TableRowDelete` – deletes the current row
- `RichTextEditor.TableToggleHeaderRow` / `RichTextEditor.TableToggleHeaderColumn` – toggle header row/column
- `RichTextEditor.TableMergeCells` / `RichTextEditor.TableSplitCell` – merge selected cells / split a merged cell

## Details

To add collapsible sections (`<details>`/`<summary>`), install the
[Tiptap details extension](https://tiptap.dev/docs/editor/extensions/nodes/details) and register all
...
// → <td>Text</td> instead of <td><p>Text</p></td>
```
...
`inline*` list items cannot hold a nested list, so **list nesting breaks** – the <kbd>Tab</kbd>
