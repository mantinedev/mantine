...
- `RichTextEditor.TableInsert` – inserts a table, opens a grid to pick its size. Enabled outside of a
  table, since that is where a new table is inserted. Supports `maxColumns`, `maxRows`,
  `withHeaderRow` and `popoverProps` props.
