...
## Virtualization (alpha)

Note that virtualization support is currently in alpha stage and the API may change in future releases.
...
`useVirtualizedCombobox` does not depend on any specific virtualization library. During the
development, it was tested with [react-virtuoso](https://virtuoso.dev/).
