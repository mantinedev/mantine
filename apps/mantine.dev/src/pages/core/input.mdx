...
## keepErrorMounted prop

By default, the error element is removed from the DOM when the `error` prop is not set. Set
`keepErrorMounted` to keep the error element mounted even when there is no error. This is useful for
`aria-live` validation announcements that require a persistent DOM node: instead of mounting a new
node every time an error appears, the existing node text is updated, which makes screen reader
...
function Demo() {
  return (
    <TextInput
      ...
      error={/* error message or null */ null}
    />
  );
}
```
...
them ([TextInput](/core/text-input/), [Textarea](/core/textarea/), [NumberInput](/core/number-input/),
[Select](/core/select/), [DateInput](/dates/date-input/), and others). This is the easiest way to apply
...
on the [theme](/theming/theme-object/) to add styles to all inputs:
