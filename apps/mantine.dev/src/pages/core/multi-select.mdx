...
Set the `withPillsReorder` prop to allow reordering pills with drag-and-drop.
Dropping a pill before or after another pill updates the component value accordingly.
Reordering is automatically disabled when `disabled` or `readOnly` is set.
...
the render callback payload onto your pill element to keep reordering working:
