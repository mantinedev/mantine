...
`leftIcon` and `rightIcon` allow adding icons to the left and right side of the button.
When an icon is added, padding on the corresponding side is reduced.

Note that `leftIcon` and `rightIcon` are flipped in [RTL](/styles/rtl) mode
(`leftIcon` is displayed on the right and `rightIcon` is displayed on the left).
