...
- `&[data-disabled]` is used to style the button when it is not actually disabled but should look like
  it is (for example, `data-disabled` should be used if you need to use [Tooltip](/core/tooltip) with a disabled `Button`
...
[Tooltip](/core/tooltip) with a disabled `Button`, you need to set the `data-disabled` prop on the `Button`
