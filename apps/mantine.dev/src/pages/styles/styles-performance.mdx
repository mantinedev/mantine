...
Applying styles with `classNames` (see the [Styles API guide](/styles/styles-api) to learn more):
...
`deduplicateInlineStyles` on [MantineProvider](/theming/mantine-provider) to
...
Currently, deduplication applies to [style props](/styles/style-props) on all components
and to responsive props on the [Flex](/core/flex) component. Components like
[Grid](/core/grid) and [SimpleGrid](/core/simple-grid) are not yet covered.
