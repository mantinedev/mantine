...
the [Button](/core/button/) component has `color`, `variant`, `size` and `radius` props that control its
appearance:
...
[Style props](/styles/style-props/) work similarly to component-specific props, but with several differences:
