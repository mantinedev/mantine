import { HooksDemos, TagsInputDemos, SliderDemos, BarChartDemos } from '@docs/demos';
...
[Slider](/core/slider) and [RangeSlider](/core/range-slider) components now support
`domain` prop that allows setting the possible range of values independently of the
`min` and `max` values:
