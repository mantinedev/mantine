...
Mantine 8.0 is a major release that includes breaking changes. In this release
breaking changes were kept to a minimum and are mostly related to third-party
packages updates and improvements in the API of extensions (`@mantine/core`
and `@mantine/hooks` packages do not have any major breaking changes).

The migration process should be straightforward and not time-consuming,
follow the [7.x → 8.x migration guide](/guides/7x-to-8x) to update
your application to Mantine 8.0.
