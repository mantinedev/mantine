...
- New [useScrollDirection](/hooks/use-scroll-direction) hook detects whether the user is currently scrolling up (`true`) or down (`false`). It handles resize events to avoid false direction changes and always uses the latest state without stale closure issues.
