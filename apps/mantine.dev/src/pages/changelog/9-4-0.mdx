...
[DayView](/schedule/day-view) and [WeekView](/schedule/week-view) now support `withSubHourGridLines`
...
[Virtual colors](/theming/colors/#virtual-colors) now support [autoContrast](/theming/theme-object/#autocontrast).
...
- [Splitter](/core/splitter) now supports `resetOnDoubleClick` prop. When enabled, double-clicking a resize handle restores the adjacent panes to their default ratio.
- [TimePicker](/dates/time-picker) now supports `closeDropdownOnPresetSelect` prop. When set, the dropdown is closed once a value is selected from the presets list.
