import { ActionBarDemos, AreaChartDemos, CandlestickChartDemos, CascaderDemos, DayViewDemos, FloatingWindowDemos, GaugeChartDemos, LightboxDemos, MatrixChartDemos, NotificationsDemos, PasswordInputDemos, ResourcesMonthViewDemos, ScatterChartDemos, StepperDemos, TipTapDemos, WaffleChartDemos, YearViewDemos } from '@docs/demos';
...
## Schedule resize interval

Time-grid Schedule views (`DayView`, `WeekView`, `ResourcesDayView`, `ResourcesWeekView`) now support
an `eventResizeInterval` prop that sets the snap step used when resizing events, independent of the
`intervalMinutes` grid size. For example, a 30-minute grid can allow 15-minute resize increments:

<Demo data={DayViewDemos.eventResizeInterval} />

## Schedule drag interval

Time-grid Schedule views (`DayView`, `WeekView`, `ResourcesDayView`, `ResourcesWeekView`) now support
an `eventDragInterval` prop that sets the snap step used when moving events by drag and drop,
independent of the `intervalMinutes` grid size. For example, a 30-minute grid can allow 15-minute
drag increments. A ghost preview shows where the event will land while dragging:
...
Schedule views (`DayView`, `WeekView`, `MonthView`, `ResourcesDayView`, `ResourcesWeekView`) now
support `withInteractiveBackgroundEvents` prop – background events (`display: 'background'`)
become clickable and trigger `onEventClick`, which makes it possible to open an edit modal for
unavailability blocks and similar events:
