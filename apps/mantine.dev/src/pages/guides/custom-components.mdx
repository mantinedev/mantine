...
2. [Default props](/theming/default-props) on theme – lower priority
...
`polymorphicFactory` is used to create [polymorphic components](/guides/polymorphic).
Use `polymorphicFactory` instead of `factory` if you need to change the root element.
For example, [Button](/core/button) component is polymorphic: the default root element is `button`,
...
For example, [Accordion](/core/accordion) component `value` and `onChange` props type
