...
Validation rules can now be async – return a `Promise<ReactNode | null>` instead of `ReactNode | null`.
When any rule is async, `form.validate()`, `form.validateField()` and `form.isValid()` return promises.
When all rules are synchronous, these methods continue to return results directly, so existing
synchronous code does not need any changes. TypeScript infers the correct return type based on
your validation rules.

If you add async rules and your code uses return values from these methods, you need to `await` them:
...
const form = useForm({
  ...
  initialValues: { name: '' },
  validate: {
    // Sync rule – validate() and isValid() return results directly
    name: (value) => (value.trim().length < 2 ? 'Too short' : null),
  },
});

// ✅ Still works – all rules are sync, so validate() returns directly
const result = form.validate();
if (result.hasErrors) { /* ... */ }

// ✅ Still works – all rules are sync, so isValid() returns a boolean
if (form.isValid()) { /* ... */ }
```

When you introduce async rules, the return types change to promises:
...
// ✅ Await required – async rule makes validate() return a Promise
const result = await form.validate();
if (result.hasErrors) { /* ... */ }

// ✅ Await required – async rule makes isValid() return a Promise
if (await form.isValid()) { /* ... */ }
```

Validation rules now receive an `AbortSignal` as the fourth argument. The signal is aborted when a newer
...
> **Note:** `form.onSubmit` does not require changes – it handles both sync and async validation
> internally and calls the success/error callbacks after validation completes.
