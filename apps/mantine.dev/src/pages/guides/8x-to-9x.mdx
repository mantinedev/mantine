...
[use-mouse](/hooks/use-mouse) hook was split into two hooks: `useMouse` and `useMousePosition`.
This change was required to fix stale ref issue in the previous implementation.

Previous usage with `document` element:
...
[use-mutation-observer](/hooks/use-mutation-observer) hook now uses the new callback ref approach.
...
useMutationObserver(
  ...
  { childList: true },
  // ❌ Third argument is no longer supported, use `useMutationObserverTarget` instead
...
// ✅ Rename hook to `useMutationObserverTarget`
useMutationObserverTarget(
  (mutations) => console.log(mutations),
  { childList: true },
  // ✅ Pass target element as the third argument
  document.getElementById('external-element')
);
```
