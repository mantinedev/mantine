...
Regular component props types include `React.ComponentPropsWithoutRef<'X'>`, where `X` is the root element
...
[Polymorphic component](/guides/polymorphic) props types don't include `React.ComponentPropsWithoutRef<'X'>`
because their root element depends on the `component` prop value.

Example of extending [polymorphic component](/guides/polymorphic) props:
...
features. It replaces the native element's `style` prop with Mantine's [style prop](/styles/style) and
...
`MantineTheme` is a type of the [theme object](/theming/theme-object). You can use it to add types
