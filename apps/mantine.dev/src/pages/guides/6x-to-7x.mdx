...
For a complete overview, please see the [7.0.0 changelog](/changelog/7-0-0).
...
the full documentation for the `@mantine/emotion` package, visit [this page](/styles/emotion).
...
`sx` and `styles` props are available in 7.x the same way as in 6.x after [setup](/styles/emotion):
...
In v7, the color scheme value is managed by [MantineProvider](/theming/mantine-provider),
and the [theme object](/theming/theme-object) no longer includes the `colorScheme` property.
...
Before getting started, we recommend going through the [styles](/styles/css-modules) documentation.
...
Note that this guide assumes you have [postcss-preset-mantine](/styles/postcss-preset) installed and configured
in your project.

### createStyles

The `createStyles` function is no longer available in 7.0. Use [CSS Modules](/styles/css-modules) instead.
...
The `sx` prop is no longer available in 7.0. Use `className` or the [style prop](/styles/style) instead.
...
Nested selectors are not supported in the [style prop](/styles/style); use `className` instead:
...
All [theme](/theming/theme-object) properties are now available as [CSS variables](/styles/css-variables). We recommend using
[CSS variables](/styles/css-variables) instead of referencing the theme object in styles.
...
The color scheme value is managed by [MantineProvider](/theming/mantine-provider),
and the [theme object](/theming/theme-object) no longer includes the `colorScheme` property.
Although it's still possible to access the color scheme value in components with the
[useMantineColorScheme](/theming/color-schemes#use-mantine-color-scheme-hook) hook,
it's not recommended to base your styles on its value. Instead, use the `light`/`dark`
[mixins](/styles/postcss-preset) or the `light-dark` CSS [function](/styles/postcss-preset#light-dark-function).
