...
[Portal](https://mantine.dev/llms/core-portal.md) component now supports `reuseTargetNode` prop which allows to reuse the same target node for all instances.
...
to your [theme](https://mantine.dev/llms/theming-theme-object.md):
...
`formRootRule` is a special rule path that can be used to [validate](https://mantine.dev/llms/form-validation.md) objects and arrays
...
New `isJSONString` and `isNotEmptyHTML` [form validators](https://mantine.dev/llms/form-validators.md):
...
[Popover](https://mantine.dev/llms/core-popover.md) now supports `onDismiss` prop, which makes it easier
...
[MantineProvider](https://mantine.dev/llms/theming-mantine-provider.md) component now supports `env` prop.
...
New [use-file-dialog](https://mantine.dev/llms/hooks-use-file-dialog.md) allows capturing one or more files from the user
...
was removed, use [React Router](https://mantine.dev/llms/guides-react-router.md) instead. To simplify maintenance,
...
* [Tooltip](https://mantine.dev/llms/core-tooltip.md) now supports customizing `middlewares`
* [ScrollArea](https://mantine.dev/llms/core-scroll-area.md) now supports `overscrollBehavior` prop
* [Affix](https://mantine.dev/llms/core-affix.md) now supports `theme.spacing` values for `position` prop
* [Anchor](https://mantine.dev/llms/core-anchor.md) now supports `underline="not-hover"` option to display underline only when the link is not hovered
