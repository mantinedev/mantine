...
Run the following script to update the `package.json` file:
...
to a specific version, you can do it manually. Open the `package.json` file; it may look
something like this:
...
Update the dependencies version in the `package.json` file to the desired version.
...
`postcss-preset-mantine` to `1.11.0`, your `package.json` file should be
updated like this:
...
version of the [@mantine/hooks package on npm](https://www.npmjs.com/package/@mantine/hooks). It
is also displayed near the Mantine logo on the [documentation home page](https://mantine.dev).

To check the latest version of the `postcss-preset-mantine` package, visit the
