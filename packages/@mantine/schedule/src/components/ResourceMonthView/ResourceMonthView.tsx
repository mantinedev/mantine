...
import classes from './ResourceMonthView.module.css';

export type ResourceMonthViewStylesNames =
  | 'resourceMonthView'
  | 'resourceMonthViewRoot'
  | 'resourceMonthViewScrollArea'
  | 'resourceMonthViewDayLabelsRow'
  | 'resourceMonthViewCorner'
  | 'resourceMonthViewDayLabel'
  | 'resourceMonthViewDayLabelWeekday'
  | 'resourceMonthViewDayLabelNumber'
  | 'resourceMonthViewRow'
  | 'resourceMonthViewResourceLabel'
  | 'resourceMonthViewRowSlots'
  | 'resourceMonthViewCell'
  | 'resourceMonthViewInner'
  | CombinedScheduleHeaderStylesNames;

export type ResourceMonthViewCssVariables = {
  resourceMonthView:
    | '--resource-month-view-radius'
    | '--resource-month-view-day-width'
    | '--resource-month-view-row-height';
};

export interface ResourceMonthViewProps
  extends BoxProps, StylesApiProps<ResourceMonthViewFactory>, ElementProps<'div'> {
...
export type ResourceMonthViewFactory = Factory<{
  props: ResourceMonthViewProps;
  ref: HTMLDivElement;
  stylesNames: ResourceMonthViewStylesNames;
  vars: ResourceMonthViewCssVariables;
}>;

const varsResolver = createVarsResolver<ResourceMonthViewFactory>(
  (_theme, { radius, dayWidth, rowHeight }) => ({
    resourceMonthView: {
      '--resource-month-view-radius': radius ? getRadius(radius) : undefined,
      '--resource-month-view-day-width': rem(dayWidth),
      '--resource-month-view-row-height': rem(rowHeight),
    },
  })
);
...
const defaultProps = {
  ...
  mode: 'default',
} satisfies Partial<ResourceMonthViewProps>;

export const ResourceMonthView = factory<ResourceMonthViewFactory>((_props) => {
  const props = useProps('ResourceMonthView', defaultProps, _props);
  const {
    classNames,
    className,
    style,
  ...
  const getStyles = useStyles<ResourceMonthViewFactory>({
    ...
    vars,
    varsResolver,
    rootSelector: 'resourceMonthView',
  });

  const { resolvedClassNames, resolvedStyles } = useResolvedStylesApi<ResourceMonthViewFactory>({
    classNames,
    styles,
    props,
  });
  ...
  const dayLabels = monthDays.map((day) => {
    ...
    return (
      <Box {...getStyles('resourceMonthViewDayLabel')} key={day} mod={{ weekend, today }}>
        ...
        <span {...getStyles('resourceMonthViewDayLabelNumber')}>{d.date()}</span>
      </Box>
    );
  });
  ...
  const rows = resources.map((resource, resourceIndex) => {
    ...
    const cells = monthDays.map((day, dayIndex) => {
      ...
      return (
        <UnstyledButton
          ...
          {...getStyles('resourceMonthViewCell')}
    ...
    return (
      <div {...getStyles('resourceMonthViewRow')} key={resource.id}>
        <div {...getStyles('resourceMonthViewResourceLabel')}>
          {renderResourceLabel ? renderResourceLabel(resource) : resource.label}
        </div>
        <div {...getStyles('resourceMonthViewRowSlots')}>{cells}</div>
      </div>
    );
  });
...
ResourceMonthView.displayName = '@mantine/schedule/ResourceMonthView';
ResourceMonthView.classes = classes;
ResourceMonthView.varsResolver = varsResolver;
