...
interface GetResourceDayViewEventsInput {
...
export interface ResourceDayViewEventsResult {
...
export function getResourceDayViewEvents({
  events,
  resources,
  date,
  startTime,
  endTime,
}: GetResourceDayViewEventsInput): ResourceDayViewEventsResult {
  const result: ResourceDayViewEventsResult = {
    regularEvents: {},
    allDayEvents: {},
  ...
  for (const event of events) {
    ...
    if (isOnDay || spansIntoDay) {
      ...
      if (!ids.has(event.id)) {
        ...
        throw new Error(
          `[@mantine/schedule] ResourceDayView: Duplicated event ids found: ${event.id}`
        );
      }
