...
import { getResourceDayViewEvents } from './get-resource-day-view-events';
...
describe('@mantine/schedule/get-resource-day-view-events', () => {
  it('returns empty arrays per resource when events is undefined', () => {
    const result = getResourceDayViewEvents({ events: undefined, resources, date: testDate });
  ...
  it('groups events by resourceId', () => {
    ...
    const result = getResourceDayViewEvents({ events, resources, date: testDate });
  ...
  it('throws on duplicate event IDs', () => {
    ...
    expect(() => getResourceDayViewEvents({ events, resources, date: testDate })).toThrow(
      '[@mantine/schedule] ResourceDayView: Duplicated event ids found: 1'
    );
  });
  ...
  it('filters by startTime and endTime', () => {
    ...
    const result = getResourceDayViewEvents({
      events,
      resources,
      date: testDate,
      startTime: '09:00',
