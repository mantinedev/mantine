...
import type { ResourceDayViewControlsRef } from './handle-resource-day-view-key-down';
import type { ResourceDayViewFactory } from './ResourceDayView';

export interface ResourceDayViewRowProps {
  resource: ScheduleResourceData;
  resourceIndex: number;
  date: string;
  slots: DayTimeInterval[];
  getStyles: GetStylesApi<ResourceDayViewFactory>;
  ...
  mode?: ScheduleMode;
  slotsRef?: ResourceDayViewControlsRef;
...
export function ResourceDayViewRow({
  resource,
  resourceIndex,
  date,
  slots,
  ...
  const items = slots.map((slot, slotIndex) => {
    ...
    return (
      <UnstyledButton
        ...
        {...getStyles('resourceDayViewRowSlot')}
        mod={{
  ...
  return (
    <Box {...getStyles('resourceDayViewRow')}>
      <Box {...getStyles('resourceDayViewResourceLabel')} mod={{ 'scrolled-x': scrolledX }}>
        {renderResourceLabel ? renderResourceLabel(resource) : resource.label}
      </Box>
      <Box
        ref={rowSlotsContainerRef}
        {...getStyles('resourceDayViewRowSlots')}
        onDragOver={
