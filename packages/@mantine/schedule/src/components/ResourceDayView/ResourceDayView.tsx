...
import { getResourceDayViewEvents } from './get-resource-day-view-events/get-resource-day-view-events';
import {
  handleResourceDayViewKeyDown,
  ResourceDayViewControlsRef,
} from './handle-resource-day-view-key-down';
import { ResourceDayViewRow } from './ResourceDayViewColumn';
import classes from './ResourceDayView.module.css';

export type ResourceDayViewStylesNames =
  | 'resourceDayView'
  | 'resourceDayViewRoot'
  | 'resourceDayViewInner'
  | 'resourceDayViewTimeLabelsRow'
  | 'resourceDayViewScrollArea'
  | 'resourceDayViewCorner'
  | 'resourceDayViewTimeLabel'
  | 'resourceDayViewResourceLabel'
  | 'resourceDayViewRow'
...
export type ResourceDayViewCssVariables = {
  resourceDayView:
    | '--resource-day-view-radius'
    | '--resource-day-view-slot-width'
    | '--resource-day-view-row-height';
};

export interface ResourceDayViewProps
  extends BoxProps, StylesApiProps<ResourceDayViewFactory>, ElementProps<'div'> {
...
export type ResourceDayViewFactory = Factory<{
  props: ResourceDayViewProps;
  ref: HTMLDivElement;
  stylesNames: ResourceDayViewStylesNames;
  vars: ResourceDayViewCssVariables;
}>;

const defaultProps = {
  __staticSelector: 'ResourceDayView',
  startTime: '00:00:00',
  endTime: '23:59:59',
  ...
  mode: 'default',
} satisfies Partial<ResourceDayViewProps>;

const varsResolver = createVarsResolver<ResourceDayViewFactory>(
  (_theme, { radius, slotWidth, rowHeight }) => ({
    resourceDayView: {
      '--resource-day-view-radius': radius ? getRadius(radius) : undefined,
      '--resource-day-view-slot-width': rem(slotWidth),
      '--resource-day-view-row-height': rem(rowHeight),
    },
  })
);

export const ResourceDayView = factory<ResourceDayViewFactory>((_props) => {
  const props = useProps('ResourceDayView', defaultProps, _props);
  const {
    classNames,
    className,
    style,
  ...
  const getStyles = useStyles<ResourceDayViewFactory>({
    ...
    varsResolver,
    attributes,
    rootSelector: 'resourceDayView',
  });

  const { resolvedClassNames, resolvedStyles } = useResolvedStylesApi<ResourceDayViewFactory>({
    classNames,
    styles,
    props,
  });
  ...
  const resourceEvents = getResourceDayViewEvents({
    date,
  ...
  const timeLabels = slots.map((interval) => {
    ...
    return (
      <Box
        {...getStyles('resourceDayViewTimeLabel')}
  ...
  const slotsRef: ResourceDayViewControlsRef = useRef<HTMLButtonElement[][]>([]);
  ...
  const rows = resources.map((resource, resourceIndex) => {
    ...
    const backgroundEventNodes = allBgEvents.map((event) => {
      ...
      const bgEventProps = {
        key: `bg-${event.id}`,
        ...getStyles('resourceDayViewBackgroundEvent', {
          style: {
  ...
  const content = (
    ...
    >
      ...
      <Box {...getStyles('resourceDayViewRoot')}>
        ...
        >
          <div {...getStyles('resourceDayViewInner')}>
            <Box {...getStyles('resourceDayViewTimeLabelsRow')} mod={{ scrolled }}>
              <div {...getStyles('resourceDayViewCorner')} key="corner">
...
ResourceDayView.displayName = '@mantine/schedule/ResourceDayView';
ResourceDayView.classes = classes;
ResourceDayView.varsResolver = varsResolver;
