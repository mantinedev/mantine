...
`alpha` function example in CSS with [postcss-preset-mantine](/styles/postcss-preset) (requires version 1.12.0 or higher):
...
`alpha` function is also exported from `@mantine/core` package. It converts color
to rgba string if CSS color value is passed for full browser support, otherwise
it uses `color-mix`:
...
New `lighten` and `darken` functions work similar to `alpha` function,
but instead of adding alpha channel they add white or black color
to the color with [color-mix](https://caniuse.com/mdn-css_types_color_color-mix).
