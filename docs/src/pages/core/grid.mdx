...
By default, `Grid` has `overflow: hidden` style on the root elements to prevent horizontal
scrollbars from appearing when the component is used inside element without padding to offset
negative margins. If you want to disable this behavior, set `overflow="visible"`:
