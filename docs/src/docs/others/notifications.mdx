...
### URL-driven notification

We used Remix `useSearchParams` hook, to listen to URL changes.
...
function Demo() {
  ...
  const notificationTypeFromURL = searchParams.get('type');

  const { notifications } = useNotifications();

  let hasNotification = notifications.find((n) => n.id === notificationId);
  ...
  useEffect(() => {
    if (messageFromURL && notificationTypeFromURL) {
      if (hasNotification) {
  ...
  }, [messageFromURL, notificationTypeFromURL]);
...
`autoClose` was set to false to make the URL the only variable in control.
