...
- `onConfirmPending` – called when confirm button is clicked and awaits any async calls causing the button to go into a loading state untill finished
...
The `onConfirmPending` function returns a promise causing the Confirm button to go into a loading state until the function is completed.

<Demo data={ModalsDemos.confirmPending} />
