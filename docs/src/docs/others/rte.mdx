...
RichTextEditor supports multiple mention groups. Each has an independant callback function. These are passed inside an array into the `mentionGroups` prop.

The structure of a mention group object is:

- `denotationChar` – The string which is used to trigger the mention
- `items` – An array of items that can be mentioned, items are objects with the structure:
  - `id` - Unique identifier in this array
  - `value` - The value of the mention item
- `onMention({denotationChar, index, id, value})` – An optional callback function for when a mention is selected from this group

<Demo data={RichTextDemos.mentions} />

**Important!** Each `denotationChar` must be unique within an instance of RichTextEditor, if multiple `denotationChar` share a value the first instance will be used.
