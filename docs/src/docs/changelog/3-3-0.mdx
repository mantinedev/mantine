...
- Components that use Popper ([Select](https://mantine.dev/core/select/), [Menu](https://mantine.dev/core/menu/), [DatePicker](https://mantine.dev/dates/date-picker/), etc.) now support an option to disable rendering within portal
...
- [Input.Wrapper](https://mantine.dev/core/input/) label now has `display: inline-block` styles, these styles will prevent input focusing when empty area above the input is clicked
