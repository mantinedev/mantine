...
- [Styles API](https://mantine.dev/styles/styles-api/) static selectors no longer use kebab case (was `.mantine-text-input-root`, now `.mantine-TextInput-root`)
...
- `AvatarsGroup` component now supports `total` prop to explicitly override the truncated length.
