...
To use dark color scheme, set `colorScheme` property in MantineProvider:
...
Dark theme is based on special colors palette in `theme.colors`,
you can change it with MantineProvider the same way as any other color:
...
```tsx
import { createUseStyles } from 'react-jss';
import { getFocusStyles, theming } from '@mantine/theme';
...
```tsx
import { createUseStyles } from 'react-jss';
import { getFontStyles, theming } from '@mantine/theme';
...
@mantine/theme package exports these utility types for sizes:
