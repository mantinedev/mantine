...
- `@mantine/styles` – [emotion](https://emotion.sh/) based css-in-js library that is used in all Mantine components. Usually this package is installed automatically and exported from `@mantine/core` – there is no need to install it separately, [learn more about createStyles here](/styles/create-styles/).
...
- `@mantine/ssr` – [server side rendering](/guides/ssr/) utilities.
- `@mantine/next` – components and ssr utils for [Next.js integration](/guides/next/).
- `gatsby-plugin-mantine` – [Gatsby plugin](/guides/gatsby/) to setup ssr.
...
[Styles API](/styles/styles-api/) lets you add styles to any internal part of a component:
...
- [Server side rendering with express server](/guides/ssr/)
