...
`@mantine/next` package exports `NextLink` component that can be used when it is not possible to
use `Link` component, for example, with [Tooltip](/core/tooltip/) component:
