...
`InputWrapperProps` extends `React.ComponentPropsWithoutRef<'div'>`, all other props are available under _Component props_ tab.

### InputWrapperStylesNames type

`InputWrapperStylesNames` type is a union with InputWrapper [Styles API](/theming/styles-api/) names:

```tsx
import type { InputWrapperStylesNames } from '@mantine/core';

const inputWrapperClasses: Partial<Record<InputWrapperStylesNames, 'string'>> = {
  label: 'wrapper-label',
  description: 'wrapper-description',
};

<InputWrapper classNames={inputWrapperClasses} />;
```
