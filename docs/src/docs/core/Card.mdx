...
`CardProps` is a generic type, it accepts either a string or component type as single type argument:

```tsx
// CardProps with html tag, will extend React.ComponentPropsWithoutRef<'a'>
type AnchorCardProps = CardProps<'a'>;

// CardProps with custom component, will extend React.ComponentPropsWithoutRef<typeof MyOtherComponent>
type CustomCardProps = CardProps<typeof MyOtherComponent>;
```
