---
group: 'mantine-core'
package: '@mantine/core'
title: Checkbox
order: 1
slug: /core/checkbox/
category: 'inputs'
description: 'Capture boolean input from user'
props: ['Checkbox']
import: "import { Checkbox } from '@mantine/core';"
source: 'mantine-core/src/components/Checkbox/Checkbox.tsx'
docs: 'core/Checkbox.mdx'
styles: ['Checkbox']
---

import { CheckboxDemos } from '@demos/core';

## Usage

<Demo data={CheckboxDemos.configurator} />

## States

<Demo data={CheckboxDemos.states} />

## Replace icon

<Demo data={CheckboxDemos.icon} />

## Sizes

Checkbox has 5 predefined sizes: xs, sm, md, lg, xl. Size defines label font-size, input width and height:

```tsx
<Checkbox size="xl" /> // -> predefined xl size
```

<Demo data={CheckboxDemos.sizes} />

## Indeterminate state

Checkbox supports indeterminate state. When `indeterminate` prop is true,
`checked` prop is ignored:

<Demo data={CheckboxDemos.indeterminate} demoProps={{ toggle: true }} />

## Controlled

```tsx
import { useState } from 'react';
import { Checkbox } from '@mantine/core';

function Demo() {
  const [checked, setChecked] = useState(false);
  return (
    <Checkbox checked={checked} onChange={(event) => setChecked(event.currentTarget.checked)} />
  );
}
```

## Get input ref

You can get input ref by setting `ref` prop:

```tsx
import { useRef } from 'react';
import { Checkbox } from '@mantine/core';

function Demo() {
  const ref = useRef();
  return <Checkbox ref={ref} />;
}
```

## Accessibility

Provide `aria-label` in case you use checkbox without label for screen reader support:

```tsx
<Checkbox /> // not ok, input is not labeled
<Checkbox label="My checkbox" /> // -> ok, input and label is connected
<Checkbox aria-label="My checkbox" /> // -> ok, label is not visible but will be announced by screen reader
```

## TypeScript

@mantine/core package exports following types associated with Checkbox:

- `CheckboxProps` – Checkbox component props, extends `React.ComponentProps<'input'>`
- `CheckboxStylesNames` – a union of styles names that can be used with [Styles API](/theming/styles-api/) (`classNames` and `styles` props)
