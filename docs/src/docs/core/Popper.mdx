...
`PopoverProps` extends `React.ComponentPropsWithoutRef<'div'>`, all other props are available under _Component props_ tab.

### PopoverStylesNames type

`PopoverStylesNames` type is a union with Popover [Styles API](/theming/styles-api/) names:

```tsx
import type { PopoverStylesNames } from '@mantine/core';

const popoverClasses: Partial<Record<PopoverStylesNames, 'string'>> = {
  header: 'popover-header',
  title: 'popover-title',
};

<Popover classNames={popoverClasses} />;
```
