...
Button has 6 variants: default, white, gradient, filled, light and outline:
...
@mantine/core package exports following types associated with Button:

- `ButtonProps` – Button component props, generic type: `ButtonProps<'button'>`
- `ButtonVariant` – a union of all variants
- `UnstyledButtonProps` – UnstyledButton component props, extends `React.ComponentPropsWithoutRef<'button'>`
- `ButtonStylesNames` – a union of styles names that can be used with [Styles API](/theming/styles-api/) (`classNames` and `styles` props)
