...
`ColorInputProps` extends `React.ComponentPropsWithoutRef<'input'>`, all other props are available under _Component props_ tab.

### ColorInputNames type

`ColorInputNames` type is a union with Autocomplete [Styles API](/theming/styles-api/) names:

```tsx
import type { ColorInputNames } from '@mantine/core';

const colorInputClasses: Partial<Record<ColorInputNames, 'string'>> = {
  swatch: 'color-input-swatch',
  input: 'color-input',
};

<Autocomplete classNames={colorInputClasses} />;
```
