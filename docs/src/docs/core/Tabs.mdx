...
```tsx
import type { TabsProps, TabProps } from '@mantine/core';
```

- `TabsProps` extends `React.ComponentPropsWithoutRef<'select'>`
- `TabProps` extends `React.ComponentPropsWithoutRef<'button'>`

All other props are available under _Component props_ tab.

### TabsStylesNames type

`TabsStylesNames` type is a union with Tabs [Styles API](/theming/styles-api/) names:

```tsx
import type { TabsStylesNames } from '@mantine/core';

const tabsClasses: Partial<Record<TabsStylesNames, 'string'>> = {
  control: 'tabs-tabControl',
  active: 'tabs-tabActive',
};
...
`TabsVariant` is a union, it includes all Tabs variants:

```tsx
import type { TabsVariant } from '@mantine/core';

const myVariant: TabsVariant = 'pills';
```
