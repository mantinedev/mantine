...
By default Modal uses random id to connect title and body via aria- attributes.
This will result in hydration mismatch if you use **opened** modal (closed modal do not require this step) during ssr.
To avoid that set static id:

```tsx
<Modal id="my-modal" opened />
```
