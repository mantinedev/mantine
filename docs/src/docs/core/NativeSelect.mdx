...
`NativeSelectProps` extends `React.ComponentPropsWithoutRef<'select'>`, all other props are available under _Component props_ tab.

### NativeSelectStylesNames type

`NativeSelectStylesNames` type is a union with NativeSelect [Styles API](/theming/styles-api/) names:
