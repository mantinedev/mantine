...
Grid component uses [use-id](/hooks/use-id/) hook to generate classes, provide static id prop to make it work with ssr:

```tsx
<Grid /> // -> will not work during ssr
<Grid id="my-grid" /> // -> works fine during ssr
```
