...
Import components props types:

```tsx
import type { GridProps, ColProps } from '@mantine/core';
```

Both `GridProps` and `ColProps` extend `React.ComponentPropsWithoutRef<'div'>`, all other props are available under _Component props_ tab.
