...
For data as array of objects, Select supports additional properties:

1. `disabled` property to disable the Select items.

<Demo data={SelectDemos.disabledItems} />

2. `group` property to group the Select items.
...
Set `creatable` prop to enable creating new select item.
`getCreateLabel` prop is also required for a `creatable` Select.
