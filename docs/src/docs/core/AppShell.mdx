...
You can use [ScrollArea](/core/scroll-area/) component with `Navbar.Section`,
set `grow` property and replace root element with `ScrollArea` component:
...
If you want to wrap Navbar component with your own custom component you will need to pass props to navbar and header:
...
@mantine/core package exports following types associated with AppShell:

- `AppShellProps` – Anchor component props, extends `React.ComponentProps<'div'>`
- `NavbarProps` – Navbar component props, extends `React.ComponentProps<'nav'>`
- `NavbarSectionProps` – Navbar.Section component props, generic type: `Navbar.Section<'div'>`
- `HeaderProps` – Navbar component props, extends `React.ComponentProps<'nav'>`
- `AppShellStylesNames` and `NavbarStylesNames` – unions of styles names which can be used with [Styles API](/theming/styles-api/) (`classNames` and `styles` props), Navbar.Section and Header components support only `root` style name and thus do not export styles names types
