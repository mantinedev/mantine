...
[View full source code](https://github.com/mantinedev/mantine/tree/master/src/mantine-demos/src/demos/packages/AppShell)
...
```tsx
import { AppShell, Header, HeaderProps, Navbar, NavbarProps } from '@mantine/core';
...
function App() {
  return (
    <AppShell
      navbar={<CustomNavbar width={{ base: 300 }} height={500} p="xs" />}
      header={<CustomHeader height={70} p="xs" />}
    >
      App content
    </AppShell>
  );
