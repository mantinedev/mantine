...
By default MultiSelect is placed at the bottom of the input and when there is not enough space for it flips to the top but.
If that behavior is unwanted to can lock it to 1 position.
