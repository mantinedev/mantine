...
import { TooltipDemos } from '@demos/core';
...
Tooltip is built with [Transition](/core/transition/) component.
...
`TooltipProps` extends `React.ComponentPropsWithoutRef<'div'>`, all other props are available under _Component props_ tab.

### TooltipStylesNames type

`TooltipStylesNames` type is a union with Tooltip [Styles API](/theming/styles-api/) names:

```tsx
import type { TooltipStylesNames } from '@mantine/core';

const tooltipClasses: Partial<Record<TooltipStylesNames, 'string'>> = {
  tooltip: 'tooltip-tooltip',
  arrow: 'tooltip-arrow',
};

<Tooltip classNames={tooltipClasses} />;
```
