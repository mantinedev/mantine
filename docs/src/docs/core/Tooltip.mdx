...
import { TooltipDemos, FloatingTooltipDemos } from '@mantine/demos';
...
A floating tooltip will always be placed on the cursor. The following props are used differently:

- **position** – tooltip position relative to the cursor – top, bottom, right or left, defaults to top
- **placement** – not available – always placed on the cursor
- **arrow** – arrow related props are not available – includes `withArrow`, `arrowDistance`, and `arrowSize`
- **controlled** – not able to override using the `opened` prop
- **allowPointerEvents** – not available

```tsx
<FloatingTooltip position="right" />
```

All available positions:

<Demo data={FloatingTooltipDemos.positions} />
