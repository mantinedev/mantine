...
`TextInputProps` extends `React.ComponentPropsWithoutRef<'input'>`, all other props are available under _Component props_ tab.

### TextInputStylesNames type

`TextInputStylesNames` type is a union with TextInput [Styles API](/theming/styles-api/) names:

```tsx
import type { TextInputStylesNames } from '@mantine/core';

const inputClasses: Partial<Record<TextInputStylesNames, 'string'>> = {
  label: 'input-label',
  description: 'input-description',
};

<TextInput classNames={inputClasses} />;
```
