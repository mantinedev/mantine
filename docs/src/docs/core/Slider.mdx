...
Both `SliderProps` and `RangeSliderProps` extend `React.ComponentPropsWithoutRef<'div'>`,
all other props are available under _Component props_ tab.

### SliderStylesNames type

`SliderStylesNames` type is a union with Slider and RangeSlider [Styles API](/theming/styles-api/) names:
