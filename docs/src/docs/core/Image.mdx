...
Radius has predefined values: xs, sm, md, lg, xl which are defined in [theme.radius](/theming/extend-theme/#spacing-radius-and-shadows).
...
`ImageProps` extends `React.ComponentPropsWithoutRef<'div'>`, all other props are available under _Component props_ tab.

### ImageStylesNames type

`ImageStylesNames` type is a union with Image [Styles API](/theming/styles-api/) names:

```tsx
import type { ImageStylesNames } from '@mantine/core';

const imageClasses: Partial<Record<ImageStylesNames, 'string'>> = {
  figure: 'image-figure',
  caption: 'image-caption',
};

<Image classNames={imageClasses} />;
```
