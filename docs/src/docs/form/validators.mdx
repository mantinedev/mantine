...
const form = useForm({
  ...
  validate: {
    maxLength: hasLength({ max: 20 }, 'Value must be 20 or less'),
    minLength: hasLength({ min: 10 }, 'Value must be 10 or more'),
    minMaxLength: hasLength({ min: 10, max: 20 }, 'Value must be between 10 and 20'),
  },
});
```
