...
```tsx
const [value, toggle] = useToggle('light', ['light', 'dark'] as const);
...
```tsx
const [value, toggle] = useToggle('light', ['light', 'dark']); // value is string
const [value, toggle] = useToggle('light', ['light', 'dark'] as const); // value is 'dark' | 'light'
const [value, toggle] = useToggle<'dark' | 'light'>('light', ['light', 'dark']); // same as above
```
