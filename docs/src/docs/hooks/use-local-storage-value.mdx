...
<PageHeader
  source="mantine-hooks/src/hooks/use-local-storage-value/use-local-storage-value.ts"
...
// 'mantine' will be assigned to value variable
...
```tsx
useLocalStorageValue<T extends string>(options: {
  key: string;
  defaultValue?: T;
