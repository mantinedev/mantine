...
use-local-storage allows you to use value from `localStorage` as react state. For `sessionStorage` use use-session-storage which accepts the same properties.
Both hooks works exactly the same as `useState`, but also writes the value to local/session storage:
