...
When rendering a component on the server that uses useMediaQuery, it is important to pass the second argument `ssrInitialValue` because without it the server may generate html that the React client cannot correctly hydrate. 
...
<Demo data={HooksDemos.useMediaQuerySSRDemo} />