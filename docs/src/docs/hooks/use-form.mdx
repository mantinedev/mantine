...
<Alert title="Deprecation Warning!" color="red" variant="filled">
  {'This hook will be removed in one of the next versions. Please switch to new '}
  <Text variant="link" inherit component="a" href="/form/use-form/">
    use-form
  </Text>
</Alert>
...
<Demo data={HooksDemos.useFormBase} />
...
`getInputProps` function returns an object with `value` (or `checked` for checkboxes), `onChange` and `error` that should be spread on input, work perfect with Mantine inputs:
...
<Demo data={HooksDemos.useFormBlur} />

### External field validation

Submit form with `test@mantine.dev` email to see external validation error:

<Demo data={HooksDemos.useFormValidation} />

### Authentication form

<Demo data={HooksDemos.useFormAuth} />
