...
import { useHotkeysHook } from '@docs/demos';
...
if you need to support other elements. For this purpose package exports `getHotkeysHandler` function
which should be used with `onKeyDown`:

<Demo data={useHotkeysHook} />
