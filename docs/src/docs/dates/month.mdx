...
import { MonthDemos, RangeCalendarDemos } from '@mantine/demos';
...
<Demo data={MonthDemos.weekendDays} />
...
All components that use `Month` under the hood also support day styles customization
with [Styles API](/styles/styles-api/). `day` selector has the following `data-` attributes:
...
[RangeCalendar](/dates/calendar/) example:

<Demo data={RangeCalendarDemos.stylesApi} />

## Add styles to month with Styles API

to change style of all element and style the active element: 
```tsx
...
## Add styles to year with Styles API
