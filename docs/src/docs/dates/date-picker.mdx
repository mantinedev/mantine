...
By passing `allowManualTyping` you stop input from beeing a readonly. User is allowed to type date manually, date is validated after losing focus on input.
